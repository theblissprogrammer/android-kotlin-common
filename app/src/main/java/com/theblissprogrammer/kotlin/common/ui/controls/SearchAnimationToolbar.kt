...
class SearchAnimationToolbar(context: Context, attrs: AttributeSet?, defStyleAttr: Int):
    ...
    private fun bindAttributes(attrs: AttributeSet?) {
        ...
        val toolbarSubtitleColor =
            typedArray.getColor(R.styleable.SearchAnimationToolbar_subtitleTextColor, Color.WHITE)
        val searchHint = typedArray.getString(R.styleable.SearchAnimationToolbar_searchHint)
        val searchBackgroundColor =
            typedArray.getColor(R.styleable.SearchAnimationToolbar_searchBackgroundColor, Color.WHITE)
    ...
    private fun inflateAndBindViews() {
        ...
        txtSearch = searchView?.findViewById(androidx.appcompat.R.id.search_src_text) as? EditText
        txtSearch?.inputType = InputType.TYPE_TEXT_FLAG_CAP_WORDS
